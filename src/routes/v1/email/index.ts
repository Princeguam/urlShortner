...
EmailRoute.post(
    ...
    asyncHandler(async (req: Request, res: Response) => {
        ...
        let emailToken = uuid.v4(); // dont save the token directly, encrypt it or even hash it
        ...
        sendEmail(
            userEmail,
            kEmailverificationSubject,
            kEmailVerificationHTMLText(user.Username, emailToken),
        );
        res.status(200).json(
            systemResponse(
                true,
...
EmailRoute.get(
    ...
    asyncHandler(async (req: Request, res: Response) => {
        ...
        sendEmail(
            user.User.Email,
            kWelcomeEmailSubject,
            kWelcomeHTMLText(user.User.Username),
        );
        res.status(200).json(
